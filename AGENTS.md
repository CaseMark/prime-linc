...
## Testing prime-linc Interactive Mode with tmux

To test prime-linc's TUI in a controlled terminal environment:

```bash
# Create tmux session with specific dimensions
tmux new-session -d -s prime-linc-test -x 80 -y 24

# Start prime-linc from source
tmux send-keys -t prime-linc-test "cd /Users/kevin/pi/prime-linc && ./prime-linc.sh" Enter

# Wait for startup, then capture output
sleep 3 && tmux capture-pane -t prime-linc-test -p

# Send input
tmux send-keys -t prime-linc-test "your prompt here" Enter

# Send special keys
tmux send-keys -t prime-linc-test Escape
tmux send-keys -t prime-linc-test C-o  # ctrl+o

# Cleanup
tmux kill-session -t prime-linc-test
```
...
- Changed `prime-linc` to open a new chat by default instead of resuming the previous session.
...
- **Internal changes (from issues)**: `Fixed foo bar ([#123](https://github.com/PrimeIntellect-ai/prime-linc/issues/123))`
- **External contributions**: `Added feature X ([#456](https://github.com/PrimeIntellect-ai/prime-linc/pull/456) by [@username](https://github.com/username))`
...
- GitHub Releases publish one standalone npm-compatible `@casemark/prime-linc` tarball staged from `packages/coding-agent` by `scripts/publish.mjs`.
...
- Build, test, pack, and smoke validation run without write credentials. The separate `github-release` environment job receives the validated artifact, creates a draft, and publishes it only after exact asset verification. Repository release immutability locks its tag and assets and generates a release attestation. See `docs/releases.md`.
