...
prime_agent_package="${PRIME_AGENT_PACKAGE:-prime-linc}"
prime_agent_cmd="${PRIME_AGENT_CMD:-prime-linc}"
...
main() {
	...
	if [ "$prime_agent_screen_enabled" = 1 ]; then
		prime_agent_screen "Installing prime-linc" "" "" ""
	else
		printf '\n\033[1m  Installing prime-linc\033[0m\n\033[2m  npm global install\033[0m\n\n'
	fi
	...
	if [ "${PRIME_AGENT_NODE_INSTALLED_STANDALONE:-0}" = 1 ]; then
		prime_agent_screen "prime-linc installed" "" "Checking your shell PATH." ""
		configure_standalone_node_path
	elif command -v "$prime_agent_cmd" >/dev/null 2>&1; then
		if [ "$prime_agent_screen_enabled" = 1 ]; then
			prime_agent_screen "prime-linc installed" "" "Run it with: $prime_agent_cmd" ""
		else
			printf '\nprime-linc was installed successfully.\n'
			printf '\nRun it with: %s\n' "$prime_agent_cmd"
		fi
	else
		if [ "$prime_agent_screen_enabled" = 1 ]; then
			prime_agent_screen "prime-linc installed" "" "PATH update needed for $prime_agent_cmd." ""
...
create_temp_dir() {
	if command -v mktemp >/dev/null 2>&1; then
		if tmp_dir=$(mktemp -d "${TMPDIR:-/tmp}/prime-linc-install.XXXXXX" 2>/dev/null); then
			printf '%s' "$tmp_dir"
			return
		fi
	fi
...
prime_agent_style_prime_agent_title() {
	...
	while :; do
		case "$text" in
			*"prime-linc"*)
				before=${text%%prime-linc*}
				rest=${text#*prime-linc}
				styled="${styled}${prime_agent_bold}${prime_agent_color_primary}${before}"
				styled="${styled}${prime_agent_bold}${prime_agent_color_primary}prime-linc${prime_agent_reset}"
				text="$rest"
				;;
			*)
...
run_preflight_checks() {
	...
	if command -v node >/dev/null 2>&1; then
		...
		if ! node -e 'const [major, minor, patch] = process.versions.node.split(".").map(Number); process.exit(major > 20 || (major === 20 && (minor > 6 || (minor === 6 && patch >= 0))) ? 0 : 1)' >/dev/null; then
			printf 'error: prime-linc requires Node.js 20.6.0 or newer. Found %s.\n' "$node_version"
			status=1
		fi
	else
		printf 'error: Node.js 20.6.0 or newer is required to install prime-linc.\n'
		status=1
	fi

	if ! command -v npm >/dev/null 2>&1; then
		printf 'error: npm is required to install prime-linc.\n'
		status=1
	fi
...
resolve_prime_agent_version() {
	...
	if ! command -v curl >/dev/null 2>&1; then
		printf 'error: curl is required to resolve the latest prime-linc version.\n' >&2
		exit 1
	fi
	...
	case "$release_channel" in
		stable|beta) ;;
		*)
			printf 'error: invalid prime-linc release channel: %s\n' "$release_channel" >&2
			exit 1
			;;
	esac
	...
	if ! prime_agent_run_quiet_with_animation \
		...
		rm -rf "$channel_dir"
		printf 'error: could not resolve latest prime-linc version from %s/%s\n' "$prime_agent_base_url" "$release_channel" >&2
		exit 1
	fi
...
normalize_version() {
	...
	case "$version" in
		"")
			printf 'error: empty prime-linc version.\n' >&2
			exit 1
			;;
		*[!0-9A-Za-z.-]*)
			printf 'error: invalid prime-linc version: %s\n' "$1" >&2
			exit 1
			;;
	esac
	printf '%s' "$version"
...
install_node_npm_interactive() {
	...
	if prime_agent_prompt_yes_no \
		...
		"Required before prime-linc can be installed." \
		"Install? [Y/n]"; then
...
Preparing prime-linc setup."
	...
	hash -r
	if [ "$prime_agent_screen_enabled" = 1 ]; then
		prime_agent_screen "Node.js and npm installed" "" "Continuing prime-linc setup." ""
	else
...
node_standalone_base_dir() {
	if [ -n "${XDG_DATA_HOME:-}" ]; then
		printf '%s/prime-linc-node' "$XDG_DATA_HOME"
	else
		printf '%s/.local/share/prime-linc-node' "$HOME"
	fi
}
...
configure_standalone_node_path() {
	...
	if shell_profile_has_standalone_node_path "$profile"; then
		if [ "$prime_agent_screen_enabled" = 1 ]; then
			prime_agent_screen "prime-linc installed" "" "Run: $(prime_agent_source_profile_command "$profile")" ""
		else
...
download_prime_agent_package() {
	...
	if ! command -v curl >/dev/null 2>&1; then
		printf 'error: curl is required to download prime-linc.\n' >&2
		exit 1
	fi
	...
	prime_agent_run_quiet_with_animation \
		...
		"prime-linc v$version" \
		curl -fsSL "$checksums_url" -o "$checksums_path"

	prime_agent_run_quiet_with_animation \
		"Downloading prime-linc" \
		"Downloading prime-linc v$version" \
...
verify_prime_agent_package_checksum() {
	...
	if command -v sha256sum >/dev/null 2>&1; then
		...
		printf 'error: sha256sum or shasum is required to verify the prime-linc download.\n' >&2
		exit 1
	fi
}
...
confirm_install() {
	...
	if prime_agent_prompt_yes_no \
		"Install prime-linc v$version globally with npm?" \
...
confirm_kernel_runtime_setup() {
	...
	if prime_agent_prompt_yes_no \
		...
		"Installs uv, Python 3.11, ipykernel, and prime-linc runtime." \
		"Prepare? [Y/n]"; then
		prime_agent_bootstrap_kernel_on_install=1
		return
	else
