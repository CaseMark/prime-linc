...
<h1 align="center">prime-linc AI</h1>

<p align="center">
  LLM provider toolkit.
</p>

Release docs use the prime-linc package name. The source workspace manifest still keeps an inherited package name until the namespace migration is complete.
...
```bash
npm install prime-linc-ai
```

TypeBox exports are re-exported from `prime-linc-ai`: `Type`, `Static`, and `TSchema`.

## Quick Start

```typescript
import { Type, getModel, stream, complete, Context, Tool, StringEnum } from 'prime-linc-ai';
...
```typescript
import { Type, Tool, StringEnum } from 'prime-linc-ai';
...
```typescript
import { stream, validateToolCall, Tool } from 'prime-linc-ai';
...
```typescript
import { readFileSync } from 'fs';
import { getModel, complete } from 'prime-linc-ai';
...
```typescript
import { getModel, streamSimple, completeSimple } from 'prime-linc-ai';
...
```typescript
import { getModel, stream } from 'prime-linc-ai';
...
```typescript
import { getProviders, getModels, getModel } from 'prime-linc-ai';
...
```typescript
import { Model, stream } from 'prime-linc-ai';
...
Use model-level `thinkingLevelMap` to describe model-specific thinking controls. Keys are prime-linc thinking levels (`off`, `minimal`, `low`, `medium`, `high`, `xhigh`). Missing keys use provider defaults, string values are sent to the provider, and `null` marks a level unsupported.
...
```typescript
import { streamAnthropic, type AnthropicOptions } from 'prime-linc-ai';
...
```typescript
import { getModel, complete, Context } from 'prime-linc-ai';
...
```typescript
import { Context, getModel, complete } from 'prime-linc-ai';
...
- OAuth login flows are not supported in browser environments. Use the `prime-linc-ai/oauth` entry point in Node.js.
...
```typescript
import { getEnvApiKey } from 'prime-linc-ai';
...
```bash
npx prime-linc-ai login              # interactive provider selection
npx prime-linc-ai login anthropic    # login to specific provider
npx prime-linc-ai list               # list available providers
```
...
The library provides login and token refresh functions via the `prime-linc-ai/oauth` entry point. Credential storage is the caller's responsibility.
...
} from 'prime-linc-ai/oauth';
```

### Login Flow Example

```typescript
import { loginGitHubCopilot } from 'prime-linc-ai/oauth';
...
```typescript
import { getModel, complete } from 'prime-linc-ai';
import { getOAuthApiKey } from 'prime-linc-ai/oauth';
...
- Add any root-level `export type` re-exports in `src/index.ts` that should remain available from `prime-linc-ai`
