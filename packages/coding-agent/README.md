...
- [Customization](#customization)
  ...
  - [Themes](#themes)
  - [prime-linc Packages](#prime-linc-packages)
...
For each built-in provider, prime-linc maintains a list of tool-capable models, updated with every release. Authenticate via subscription (`/login`) or API key, then select any model from that provider via `/model` (or Ctrl+L).
...
On Windows Terminal, `Alt+Enter` is fullscreen by default. Remap it in [docs/terminal-setup.md](docs/terminal-setup.md) so prime-linc can receive the follow-up shortcut.
...
prime-linc -r [path|id]        # Browse past sessions or resume one directly
prime-linc --no-session        # Ephemeral mode (don't save)
prime-linc --fork <path|id>    # Fork specific session file or ID into a new session
```
...
prime-linc stable builds fetch `https://pub-728493de92a943e2a9b2d17b4719f318.r2.dev/latest.json` to check whether a newer version exists. Beta builds fetch `beta.json` and remain on the beta channel. Override the base URL with `PRIME_AGENT_DOWNLOAD_BASE_URL`. Disable version checks with `PI_SKIP_VERSION_CHECK=1`.
...
TypeScript modules that extend prime-linc with custom tools, commands, keyboard shortcuts, event handlers, and UI components.
...
The default export can also be `async`. prime-linc waits for async extension factories before startup continues, which is useful for one-time initialization such as fetching remote model lists before calling `pi.registerProvider()`.
...
- Make prime-linc look like Claude Code
...
> **Security:** prime-linc packages run with full system access. Extensions execute arbitrary code, and skills can instruct the model to perform any action including running executables. Review source code before installing third-party packages.

```bash
prime-linc package install npm:@foo/prime-linc-tools
prime-linc package install npm:@foo/prime-linc-tools@1.2.3  # pinned version
prime-linc package install git:github.com/user/repo
prime-linc package install git:github.com/user/repo@v1       # tag or commit
prime-linc package install git:git@github.com:user/repo
prime-linc package install https://github.com/user/repo
prime-linc package install ssh://git@github.com/user/repo
prime-linc package remove npm:@foo/prime-linc-tools
prime-linc package list
prime-linc package update                                  # update packages, except pinned versions
prime-linc package update npm:@foo/prime-linc-tools       # update one package
prime-linc update                                          # update prime-linc
prime-linc update --force                                  # reinstall prime-linc even if current
...
```json
{
  "name": "my-prime-linc-package",
  "keywords": ["prime-linc-package"],
  "pi": {
    "extensions": ["./extensions"],
    "skills": ["./skills"],
    "prompts": ["./prompts"],
...
Without a `pi` manifest, prime-linc auto-discovers from conventional directories (`extensions/`, `skills/`, `prompts/`, `themes/`).
...
prime-linc is forked from [pi-mono](https://github.com/badlogic/pi-mono) by Mario Zechner and keeps MIT attribution in the root license.

The package architecture, extension model, and source package names still reflect that upstream lineage while the distributed command and release artifacts are branded for prime-linc.

## CLI Reference

```bash
prime-linc [options] [@files...] [messages...]
```

Run `prime-linc help` for the command list and `prime-linc help <command>` for details.

### Agent Commands

```bash
prime-linc agents                         # Search running, idle, and inactive sessions
prime-linc list [--all]                   # List active or saved agents
prime-linc attach <agent>                 # Attach the interactive UI
prime-linc stop <agent>                   # Stop one agent
prime-linc rename <agent> <name>          # Rename an agent
prime-linc send <agent> <message>         # Send an agent-to-agent message
prime-linc status                         # Show background service status
prime-linc doctor [--fix]                 # Inspect or safely clean up background services
prime-linc shutdown [--force]             # Stop every agent, worker, and background service
```
...
```bash
prime-linc schedule list [--all] [agent]
prime-linc schedule add <agent> <schedule> -- <message>
prime-linc schedule cancel <job-id>
```
...
```bash
prime-linc package install <source> [--local]
prime-linc package remove <source> [--local]
prime-linc package list
prime-linc package update [source]
prime-linc update [--force]                   # Update prime-linc itself
prime-linc config                             # Enable/disable package resources
```
...
In print mode, prime-linc also reads piped stdin and merges it into the initial prompt:

```bash
cat README.md | prime-linc -p "Summarize this text"
```
...
Use `prime-linc model list [search]` to list available models.
...
Use `prime-linc session export <file> [output]` to export a saved session to HTML.
...
```bash
prime-linc @prompt.md "Answer this"
prime-linc -p @screenshot.png "What's in this image?"
prime-linc @code.ts @test.ts "Review these files"
```
...
prime-linc "List all .ts files in src/"

# Non-interactive
prime-linc -p "Summarize this codebase"

# Non-interactive with piped stdin
cat README.md | prime-linc -p "Summarize this text"

# Different model
prime-linc --provider openai --model gpt-4o "Help me refactor"

# Model with provider prefix (no --provider needed)
prime-linc --model openai/gpt-4o "Help me refactor"

# Model with thinking level shorthand
prime-linc --model sonnet:high "Solve this complex problem"

# Limit model cycling
prime-linc --models "claude-*,gpt-4o"

# Restrict to the built-in IPython tool
prime-linc --tools ipython -p "Review the code"

# High thinking level
prime-linc --thinking high "Solve this complex problem"
```
...
| `PI_SKIP_VERSION_CHECK` | Skip the prime-linc version update check at startup. This prevents the release manifest request |
| `PRIME_AGENT_DOWNLOAD_BASE_URL` | Override the prime-linc release manifest and tarball base URL |
...
- [prime-linc AI](../ai): Core LLM toolkit
- [prime-linc Core](../agent): Agent framework
- [prime-linc TUI](../tui): Terminal UI components
