...
The extension factory can also be `async`. For dynamic model discovery, fetch and register models in the factory instead of `session_start`. prime-linc waits for the factory before startup continues, so the provider is available during interactive startup and to `prime-linc model list`.
...
models: [{
  ...
  // ...
  reasoning: true,
  thinkingLevelMap: {              // map prime-linc levels to provider values; null hides unsupported levels
    minimal: null,
    low: null,
    medium: null,
    high: "default",
...
interface ProviderModelConfig {
  ...
  /** Maps prime-linc thinking levels to provider/model-specific values; null marks a level unsupported. */
