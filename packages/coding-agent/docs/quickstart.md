...
This page gets you from install to a useful first prime-linc session.
...
The source runner preserves the directory from which it is invoked, so you can also call `/path/to/prime-linc/prime-linc.sh` from another project.

## Authenticate

prime-linc can use subscription providers through `/login`, or API-key providers through environment variables or its auth file.

### Option 1: Subscription Login

Start prime-linc and run:
...
Set an API key before launching prime-linc:
...
## Give prime-linc Project Instructions

prime-linc loads context files at startup. Add an `AGENTS.md` file to tell it how to work in a project:
...
```bash
prime-linc @README.md "Summarize this"
prime-linc @src/app.ts @src/app.test.ts "Review these together"
```
...
prime-linc -r [path|id]        # Browse sessions or open a specific session
```

Inside prime-linc, use `/resume`, `/new`, `/tree`, `/fork`, and `/clone` to manage sessions. Persistent sessions run in worker processes, so closing the TUI detaches from the agent rather than necessarily stopping it. Use `prime-linc agents` to inspect or reattach to active work.
...
- [Using prime-linc](usage.md) - interactive mode, slash commands, sessions, context files, and CLI reference.
- [Providers](providers.md) - authentication and model setup.
- [Settings](settings.md) - global and project configuration.
- [Keybindings](keybindings.md) - shortcuts and customization.
- [prime-linc Packages](packages.md) - install shared extensions, skills, prompts, and themes.
