> prime-linc can help you use the SDK. Ask it to build an integration for your use case.

# SDK

The SDK provides programmatic access to prime-linc's capabilities. Use it to embed prime-linc in other applications, build custom interfaces, or integrate with automated workflows.
...
- If you omit `tools`, prime-linc automatically creates them with the correct `cwd`
...
```bash
prime-linc --mode rpc --no-session
```
