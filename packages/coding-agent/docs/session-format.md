...
prime-linc also supports deleting sessions interactively from `/resume` (select a session and press `Ctrl+D`, then confirm). When available, prime-linc uses the `trash` CLI to avoid permanent deletion.
...
### Base Message Types (from prime-linc AI)
...
### Extended Message Types (from prime-linc Coding Agent)
...
- `fromHook`: `true` if generated by an extension, `false`/`undefined` if generated by prime-linc (legacy field name)
