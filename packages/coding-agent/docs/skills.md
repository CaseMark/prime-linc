> prime-linc can create skills. Ask it to build one for your use case.

# Skills

Skills are self-contained capability packages that prime-linc loads on demand. A skill provides specialized workflows, setup instructions, helper scripts, and reference documentation for specific tasks.

prime-linc implements the [Agent Skills standard](https://agentskills.io/specification), warning about violations but remaining lenient. It also supports Python-backed skills: a superset of markdown skills that install Python packages into the persistent IPython kernel.
...
- [Creating Skills with prime-linc](#creating-skills-with-prime-linc)
...
- Built-in: `skills/` shipped with the prime-linc package (lowest precedence)
...
prime-linc ships with built-in skills that load by default:
...
1. At startup, prime-linc scans skill locations and extracts names, descriptions, type, and file locations
...
For `web-search`, prime-linc exposes `web_search` in IPython. If the module defines `run()`, the module is wrapped as an async callable:
...
Python skills are installed editable into the kernel venv during kernel setup. By default this is `~/.prime/agent/kernel-venv`; set `PRIME_AGENT_KERNEL_VENV` to override it. If `pyproject.toml` changes, prime-linc rebuilds the kernel venv so dependency changes are picked up.

If you set `PRIME_AGENT_KERNEL_PYTHON`, prime-linc does not install packages into that environment. The Python must already have `ipykernel`, `prime-agent-runtime`, and the default runtime packages installed. Missing Python skill imports are disabled with a warning and calling the skill raises a `RuntimeError`.
...
## Creating Skills with prime-linc

prime-linc ships with a built-in `skill-creator` skill that teaches the agent both the Agent Skills format and the Python-backed package contract. You can ask for a skill in normal language:
...
Use `/reload` to rediscover new or edited skill metadata. Start a fresh prime-linc session after adding a Python-backed skill so kernel setup can install and import the package.
...
prime-linc validates skills against the Agent Skills standard. Most issues produce warnings but still load the skill:
