> prime-linc can create themes. Ask it to build one for your setup.
...
On first run, prime-linc detects your terminal background and defaults to `dark` or `light`.
...
```json
{
  "$schema": "https://raw.githubusercontent.com/PrimeIntellect-ai/prime-linc/main/packages/coding-agent/src/modes/interactive/theme/theme-schema.json",
  "name": "my-theme",
  "vars": {
    "primary": "#00aaff",
    "secondary": 242
...
**Hot reload:** When you edit the currently active custom theme file, prime-linc reloads it automatically for immediate visual feedback.
...
prime-linc uses 24-bit RGB colors. Most modern terminals support this (iTerm2, Kitty, WezTerm, Windows Terminal, VS Code). For older terminals with only 256-color support, prime-linc falls back to the nearest approximation.
