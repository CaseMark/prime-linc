> prime-linc can create TUI components. Ask it to build one for your use case.
...
prime-linc shows compact image metadata and does not emit terminal graphics for images.
...
Customize the inline working indicator shown while prime-linc is streaming a response.
...
// Restore prime-linc's default spinner
ctx.ui.setWorkingIndicator();
```
