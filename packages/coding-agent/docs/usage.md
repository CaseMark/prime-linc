...
On Windows Terminal, Alt+Enter is fullscreen by default. Remap it as described in [Terminal setup](terminal-setup.md) if you want prime-linc to receive the shortcut.
...
prime-linc -r [path|id]        # Browse sessions or resume one directly
prime-linc --no-session        # Ephemeral mode; do not save
prime-linc --fork <path|id>    # Fork a session into a new session file
```
...
Normal interactive sessions are persistent agents backed by isolated worker processes. Closing the TUI detaches the client; use `prime-linc agents`, `prime-linc list`, or `prime-linc attach <agent>` to find and reattach to running work. `prime-linc stop <agent>` stops one root agent, while `prime-linc shutdown` stops all workers and the local supervisor.
...
prime-linc agents
prime-linc list [--all]
prime-linc attach <agent>
prime-linc stop <agent>
prime-linc rename <agent> <name>
prime-linc send <agent> <message>
prime-linc schedule <list|add|cancel>
prime-linc status
prime-linc doctor [--fix]
prime-linc shutdown [--force]
...
prime-linc update [--force]
prime-linc config
```

See [prime-linc Packages](packages.md) for package sources and security notes.
...
Use `prime-linc session export <file> [output]` to export a session to HTML.
...
```bash
prime-linc --no-extensions -e ./my-extension.ts
```
...
prime-linc keeps the model-facing tool surface small while making the IPython runtime powerful and composable. The built-in `ipython` tool provides durable state, project command execution, Python skills, MCP-backed integrations, and the native `rlm` delegation API without presenting each capability as a separate model tool.
...
Extensions, skills, prompt templates, themes, and prime-linc packages remain the primary customization surfaces. They can add project-specific workflows, custom tools and UI, permission policies, provider integrations, and orchestration patterns around the built-in runtime.

prime-linc preserves MIT attribution to pi-mono for its upstream lineage, but upstream Pi product claims and limitations do not describe the current prime-linc architecture.
