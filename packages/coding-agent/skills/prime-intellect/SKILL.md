...
Prime Intellect is an open superintelligence lab building open-source AGI infrastructure: a platform for RL environments, evaluations, post-training, inference, and globally distributed GPU compute. The `prime` CLI is the default way to interface with every product below. (prime-linc itself is a case.dev-native fork of Prime Agent; use case.dev auth/tools for legal workflows.)
