	...
	switch (command) {
		...
		case "update": {
			...
			if (hasLegacySelfTarget && hasLegacyPackageTarget) {
				return fail(
					"prime-linc and package updates are now separate.",
...
function rejectRemovedCommand(args: string[]): PublicCommandResult {
	...
	if (command === "daemon") {
		replacement = 'Run "prime-linc help" to see the agent commands.';
	} else if (command === "app" && subcommand === "update") {
		replacement = 'Use "prime-linc update".';
	} else if (command === "install") {
		replacement = 'Use "prime-linc package install".';
	} else if (command === "remove" || command === "uninstall") {
		replacement = 'Use "prime-linc package remove".';
	} else if (command === "manage") {
		replacement = 'Use "prime-linc agents".';
	}
...
async function runPackage(args: string[]): Promise<PublicCommandResult> {
	...
	if (subcommand === "update") {
		if (
			...
			return fail(
				'Package updates accept only an optional source. Use "prime-linc update --force" to update prime-linc.',
			);
		}
		if (rest.length > 1) {
		...
		if (rest[0] && isSelfUpdateSource(rest[0])) {
			return fail('Use "prime-linc update" to update prime-linc.');
		}
