...
import { createCasedevExtension } from "../casedev/provider.js";
...
export async function createAgentSessionServices(
	...
	const herdrFactories = skipHerdrReporter
		? []
		: [createHerdrAgentStateExtension(() => resourceLoader.getLoadedExtensionPaths())];
	const casedevFactory = createCasedevExtension({
		cwd,
		getApiKey: () => modelRegistry.getApiKeyForProvider("casedev"),
	});
	const builtinExtensionFactories = [...herdrFactories, casedevFactory];
