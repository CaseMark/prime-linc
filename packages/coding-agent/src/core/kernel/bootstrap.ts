...
async function ensureUv(options: EnsureKernelPythonOptions): Promise<string> {
	...
	if (!shouldInstallUv) {
		throw new Error(
			`uv is required to set up the Python kernel. Install uv yourself: ${UV_INSTALL_COMMAND}, ` +
				"or set PRIME_AGENT_INSTALL_UV=1 to let prime-linc run that installer.",
		);
	}
	...
	try {
		...
		throw new Error(
			`couldn't install uv from astral.sh; install it yourself: ${UV_INSTALL_COMMAND}, then re-run prime-linc. ${errorMessage(error)}`,
		);
	}
...
async function confirmUvInstall(): Promise<boolean> {
	...
	try {
		const answer = (await rl.question("prime-linc needs uv to set up Python. Install uv from astral.sh now? [Y/n] "))
			.trim()
			.toLowerCase();
...
function formatBootstrapFailure(error: unknown): Error {
	return new Error(
		`Failed to set up the Python kernel runtime. ${errorMessage(error)}\n` +
			"First-time setup needs internet to install uv, Python, ipykernel, prime-agent-runtime, and default Python packages; once set up, prime-linc runs offline. " +
