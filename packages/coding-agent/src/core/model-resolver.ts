...
import { CASEDEV_DEFAULT_MODEL_ID, CASEDEV_PROVIDER_ID } from "../casedev/constants.js";
...
function findPreferredDefaultModel(availableModels: Model<Api>[]): Model<Api> | undefined {
	const caseDevDefault = availableModels.find(
		(model) => model.provider === CASEDEV_PROVIDER_ID && model.id === CASEDEV_DEFAULT_MODEL_ID,
	);
	if (caseDevDefault) {
		return caseDevDefault;
	}

	const firstCaseDev = availableModels.find((model) => model.provider === CASEDEV_PROVIDER_ID);
	if (firstCaseDev) {
		return firstCaseDev;
	}

	// case.dev is the product default. Only fall through when no case.dev models are authenticated.
