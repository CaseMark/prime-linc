...
export const PRIME_AGENT_TRACES_PROVIDER_ID = "prime-linc-traces";
export const PRIME_AGENT_TRACES_PROVIDER_NAME = "prime-linc Traces";
...
export async function loginPrimeAgentTraces(
	...
	if (config.apiKey) {
		...
		callbacks.onProgress?.(
			`Existing Prime CLI key cannot upload prime-linc traces (${formatAccessFailure(access)}). Starting browser login...`,
		);
	} else {
		callbacks.onProgress?.("No Prime CLI API key found. Starting browser login...");
	}
	...
	throwIfCancelled(callbacks.signal);
	callbacks.onProgress?.("Checking prime-linc trace access...");
	...
	if (!access.ok) {
		throw new Error(`Prime API key does not have prime-linc trace access (${formatAccessFailure(access)})`);
	}
