...
import { applyCasedevEnvFromCliConfig } from "./casedev/auth.js";
...
// `prime-linc agents` opens the agents view directly.
...
export function shouldOpenAgentsViewForDaemonInteractive(options: AgentsViewStartupDecision): boolean {
	return (
		...
		// `prime-linc` opens a new chat by default; the unified agents view is reached via
...
const STARTUP_SESSION_LOSS_COPY: DaemonSessionLossCopy = {
	busyDetail(count) {
		...
		return `A background service from a different prime-linc version is running with ${count} busy ${noun}. Stopping it will terminate ${pronoun}.`;
	},
	unlistableDetail:
		"A background service from a different prime-linc version is running and its sessions could not be listed. Stopping it may terminate active sessions.",
	question: "Stop it and continue?",
	nonTtyHint: 'Run "prime-linc shutdown" to stop it, then retry.',
};
...
async function takeOverStaleDaemonOrExit(socketPath: string): Promise<DaemonReadyResult> {
	...
	if (!(await shutdownDaemonAndWait(socketPath))) {
		console.error(
			chalk.red(`Could not stop the background service on ${socketPath}. Run "prime-linc shutdown" and retry.`),
		);
		process.exit(1);
	}
