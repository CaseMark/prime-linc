/**
 * Namespaced `_meta` payloads for prime-linc capabilities that ACP has no
 ...
 * clients ignore these keys; a prime-linc-aware client (or the verifiers
 * harness) reads them. Never add non-standard fields to an ACP object root.
 */

/** Reverse-domain namespace for every prime-linc `_meta` payload. */
export const PRIME_AGENT_META_NAMESPACE = "dev.case.prime-linc";
...
export interface PrimeAgentCwdMeta {
	...
	requested: string;
	/** The cwd prime-linc is actually running in, fixed at startup. */
	actual: string;
}
...
/** Wrap a prime-linc payload in its reverse-domain `_meta` envelope. */
