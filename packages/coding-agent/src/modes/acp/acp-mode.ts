 ...
 *
 * prime-linc acts as an ACP agent over NDJSON on stdio, driving an
 * `AgentConnection` in-process. It deliberately does not shell out to RPC mode
 * and translate: prime-linc's differentiators (IPython-only tools, subagents,
...
/**
 * Split ACP prompt blocks into the text and images prime-linc accepts.
 *
...
export async function runAcpModeWithConnection(
	...
	const handle = acp
		...
		.onRequest("initialize", async () => ({
			...
			agentInfo: { name: "prime-linc", title: "prime-linc", version: VERSION },
			// Advertise prime-linc extras under a namespaced key: ACP reserves
		...
		.onRequest("session/new", async (ctx: any) => {
			...
			if (session) {
				throw new Error(
					"prime-linc ACP mode hosts one session per connection; " +
						"start another prime-linc process for a second session",
				);
			}
			// prime-linc's cwd is fixed at startup by the session it was launched
			...
			// Subscribe for the session lifetime, not per prompt turn: prime-linc
		...
		.onRequest("session/prompt", async (ctx: any) => {
			...
			try {
				...
				if (failure && !abort.signal.aborted) {
					throw new Error(`prime-linc turn failed: ${failure}`);
				}
