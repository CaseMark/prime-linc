...
const SUMMARY_MODEL_PROVIDER = CASEDEV_PROVIDER_ID;
const SUMMARY_MODEL_ID = CASEDEV_DEFAULT_MODEL_ID;
...
export function resolveSummaryModel(registry: ModelRegistry): Model<Api> | undefined {
	...
	const fallback = registry.getAll().find((model) => model.provider === CASEDEV_PROVIDER_ID);
	if (fallback && registry.hasConfiguredAuth(fallback)) {
		return fallback;
	}
	return undefined;
}
