...
import { mirrorCasedevCliKey } from "../../casedev/auth.js";
import { CASEDEV_PROVIDER_ID } from "../../casedev/constants.js";
...
export class ProviderAuthFlows {
	...
	async runPrimeAgentTracesLogin(): Promise<AuthenticationResult> {
		...
		try {
			...
			if (result.source === "manual") {
				browserAbort.abort();
				dialog.showProgress("Checking prime-linc trace access...");
				...
				if (!access.ok) {
					const status = access.status === undefined ? "" : `HTTP ${access.status}: `;
					throw new Error(`Prime API key does not have prime-linc trace access (${status}${access.message})`);
				}
			}
	...
	private async showApiKeyLoginDialog(
		...
		try {
			...
			if (providerId === CASEDEV_PROVIDER_ID) {
				mirrorCasedevCliKey(apiKey);
			}
