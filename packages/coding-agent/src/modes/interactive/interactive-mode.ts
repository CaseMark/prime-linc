...
import { CASEDEV_PROVIDER_NAME } from "../../casedev/constants.js";
import { resolveCasedevPostLoginModelAction } from "../../casedev/model-selection.js";
import {
...
export class InteractiveMode {
	...
	private async runOnboardingFlow(showPrimeCliSplash = this.shouldRunPrimeCliOnboardingSplash()): Promise<void> {
		...
		splash.showProgress(`Signing in to ${CASEDEV_PROVIDER_NAME}...`);
		const authResult = await this.createAuthFlows().loginProvider({
			id: "casedev",
	...
	private async prepareForModelSelectionAfterLogin(authResult: AuthenticationResult): Promise<boolean> {
		...
		const resolveAction = (registry: Pick<typeof this.modelRegistry, "find" | "getAll">) => {
			const casedevAction = resolveCasedevPostLoginModelAction(authResult, selectedModel, registry);
			if (casedevAction.openModelPicker) {
				return { action: casedevAction, label: CASEDEV_PROVIDER_NAME };
			}
			const primeAction = resolvePrimeInferencePostLoginModelAction(authResult, selectedModel, registry);
			if (primeAction.openModelPicker) {
				return { action: primeAction, label: "Prime Inference" };
			}
			return undefined;
		};

		let resolved = resolveAction(this.modelRegistry);
		if (!resolved) {
			return false;
		}
		...
		if (resolved.action.fallbackModel) {
			try {
				await this.applySelectedModel(resolved.action.fallbackModel);
				await this.settingsManager.flush();
			} catch (error) {
				this.showError(
					`${resolved.label} login succeeded, but the default model could not be selected: ${error instanceof Error ? error.message : String(error)}`,
				);
			}
		} else if (!selectedModel) {
			this.showError(`${resolved.label} login succeeded, but the default model is unavailable.`);
		}
