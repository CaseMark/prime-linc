...
describe("herdrAgentStateExtension", () => {
	...
	it("reports lifecycle state to the herdr socket", async () => {
		...
		process.env.PRIME_LINC_CODING_AGENT_DIR = tempDir;
		...
		expect(requests[0]?.params.agent).toBe("prime-linc");
		...
		expect(requests[3]?.params.agent).toBe("prime-linc");
	});
