...
describe("InteractiveMode splash cwd display", () => {
	test("formats home-relative cwd paths", () => {
		...
		expect(formatSplashCwd(path.join(homedir(), "pi", "prime-linc"))).toBe("~/pi/prime-linc");
	});

	test("keeps worktree paths as cwd paths instead of repo branch labels", () => {
		expect(formatSplashCwd(path.join(homedir(), "pi", "prime-linc", ".worktrees", "improve-onboarding"))).toBe(
			"~/pi/prime-linc/.worktrees/improve-onboarding",
		);
	});
});
