...
describe("package commands", () => {
	...
	it("uses global npmCommand and the release manifest install spec for forced self updates", async () => {
		...
		const tarballUrl = "https://downloads.example.test/prime-linc/prime-linc-current.tgz";
...
`,
	...
	it("installs the prime-linc tarball from the update manifest during self-update", async () => {
		const globalPrefix = join(tempDir, "global-prefix");
		const selfPackageDir = join(globalPrefix, "lib", "node_modules", "@earendil-works", "pi-coding-agent");
		const fakeNpmPath = join(tempDir, "fake-npm.cjs");
		const recordPath = join(tempDir, "self-update.json");
		const baseUrl = "https://downloads.example.test/prime-linc";
		const tarballPath = "releases/v0.73.0/prime-linc-0.73.0.tgz";
		mkdirSync(selfPackageDir, { recursive: true });
		writeFileSync(
			fakeNpmPath,
			`const fs=require("node:fs"),path=require("node:path"),args=process.argv.slice(2),prefix=args[args.indexOf("--prefix")+1];
...
`,
		...
		vi.stubGlobal(
			"fetch",
			vi.fn(async () => Response.json({ package: "prime-linc", tarball: tarballPath, version: "0.73.0" })),
		);
	...
	it("does not self-update when the same-version manifest uses the prime-linc package alias", async () => {
		const globalPrefix = join(tempDir, "global-prefix");
		const selfPackageDir = join(globalPrefix, "lib", "node_modules", "@earendil-works", "pi-coding-agent");
		const fakeNpmPath = join(tempDir, "fake-npm.cjs");
		const recordPath = join(tempDir, "self-update.json");
...
`,
		...
		vi.stubGlobal(
			...
			vi.fn(async () =>
				Response.json({
					package: "prime-linc",
					tarball: "releases/current/prime-linc.tgz",
					version: VERSION,
				}),
			),
		);
