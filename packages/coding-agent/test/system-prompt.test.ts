...
describe("buildSystemPrompt", () => {
	test("injects compact global harness context and refine guidance by default", () => {
		...
		expect(prompt).toContain("Local continual harness entries belong to this prime-linc session");
		...
		expect(prompt).toContain("[global:validation] Validation (repo/prime-linc, v2): Run `npm run check`");
	...
	test("uses the model-agnostic rlm harness prompt", () => {
		...
		expect(prompt).toContain("You are prime-linc, a case.dev-native RLM agent for coding and legal AI workflows.");
