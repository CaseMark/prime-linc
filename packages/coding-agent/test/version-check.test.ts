...
describe("version checks", () => {
	...
	it("uses the prime-linc release manifest with a prime-linc user agent", async () => {
		...
		expect(fetchMock).toHaveBeenCalledWith(
			...
			expect.objectContaining({
				headers: expect.objectContaining({
					"User-Agent": expect.stringMatching(/^prime-linc\/1\.2\.3 /),
	...
	it("returns the active package and tarball install spec from the release manifest", async () => {
		const fetchMock = vi.fn(async () =>
			Response.json({
				package: "prime-linc",
				tarball: "releases/v1.2.4/prime-linc-1.2.4.tgz",
				version: "v1.2.4",
			}),
		);
		vi.stubGlobal("fetch", fetchMock);
		...
		await expect(getLatestPiRelease("1.2.3")).resolves.toEqual({
			installSpec: `${defaultPrimeAgentDownloadBaseUrl}/releases/v1.2.4/prime-linc-1.2.4.tgz`,
			packageName: "prime-linc",
			version: "1.2.4",
		});
	});
