...
preview_dir=$(mktemp -d "${TMPDIR:-/tmp}/prime-linc-splash-preview.XXXXXX")
...
i=0
while [ "$i" -lt "$1" ]; do
	prime_agent_screen "Installing prime-linc" "Downloading prime-linc$(prime_agent_pulse)" "Fetching the verified package." ""
	sleep 0.18
	i=$((i + 1))
done
EOF
